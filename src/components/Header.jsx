                    ...
                    <div className={`p-5 ${darkMode ? 'text-light' : 'text-oxfordBlue'} text-sm transition delay-300 duration-300 ease-in-out`}>
                        ...
                        {isLoggedIn ? (
                            <ul className='flex md:hidden sm:hidden'>
                                <li className='lg:mr-5 md:mr-5 hover:text-forestGreen'><Link to="/dashboard">Dashboard</Link></li>
                    ...
                    <div className={`p-5 ${darkMode ? 'text-light' : 'text-oxfordBlue'} text-sm`}>
                        <ul className='sm:hidden flex items-center sm:flex-row-reverse'>
                            ...
                            {isLoggedIn ?
                                (
                                    <>
                                        <li className='lg:mr-5 md:mr-5'><button onClick={logOut} className='border-solid border border-lightGreen p-3 px-8 rounded-lg hover:bg-forestGreen hover:text-light md:p-2 md:px-5 md:rounded-lg sm:text-sm'>Logout</button></li>
                                        {/* <li className='lg:mr-5'><Link to="/login" className='bg-forestGreen p-3 px-8 text-semiLight rounded-lg hover:bg-lightGreen md:p-2 md:px-5 md:rounded-lg'>Login</Link></li> */}
                                    </>
                                ) : (
                                    <>
                                        <li className='lg:mr-5 md:mr-5'><Link to="/signup" className='border-solid border border-lightGreen p-3 px-8 rounded-lg hover:bg-forestGreen hover:text-light md:p-2 md:px-5 md:rounded-lg sm:text-sm'>Sign up</Link></li>
                                        <li className='lg:mr-5'><Link to="/login" className='bg-forestGreen p-3 px-8 text-semiLight rounded-lg hover:bg-lightGreen md:p-2 md:px-5 md:rounded-lg'>Login</Link></li>
                                    </>
                                )}
                        </ul>
